...
const defaultSource = path.resolve(
  ...
  'temp',
  'public_scores',
  'public_brand_rankings_2026-04-23.json',
);
...
export async function importArtifact({ source, root = repoRoot }) {
  ...
  const snapshotFilename = `public_brand_rankings.${slug}.json`;
  const latestFilename = 'public_brand_rankings.latest.json';
  const metadataFilename = 'public_brand_rankings.metadata.json';
